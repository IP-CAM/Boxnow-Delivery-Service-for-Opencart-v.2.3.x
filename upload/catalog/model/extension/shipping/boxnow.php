	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$this->session->data['boxnow_name']			
			 ...
			 $boxnow_weight_value = $this->config->get('boxnow_weight_value');
 
			 // Initialize the cost
			 $cost = 0;
 
			 // Loop through weight ranges and calculate cost
			foreach ($boxnow_weight_value as $weight_range) {
		   ...
		   // Check if the total weight falls within this range
			...
			break; // Exit the loop once a matching range is found
			  ...
			  if ($this->config->get('boxnow_free_shipping') && $this->cart->getSubTotal() >= $this->config->get('boxnow_free_shipping')) {
				  $cost = 0;
			  }
			...
			$quote_data['boxnow'] = array(
				...
				'boxnow_weight'=> $this->config->get('boxnow_weight_value'),
